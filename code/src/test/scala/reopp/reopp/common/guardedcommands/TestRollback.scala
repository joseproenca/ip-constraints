...
//        println("searching hotel, counter "+counter_asked)
    ...
    val cancelSrch = Function("cancelSrch"){
      case x: (String,Any) => {counter_cancel+=1; request_cancel = x._1}
      case x => {println("******** UNKNOWN x: "+x+":"+x.getClass)}
    }
...
//    println("cancelled: " +counter_cancel)
  ...
  describe ("Rollback (ChocoDyn) test -") {  // before: ChocoX - now not working (deprecatd), and replaced by ChocoDyn.
    // STATE
    var counter_asked = 0
    var fstRequest = "a"
    var counter_cancel = 0
...
//                println("***** "+l.toString + " has ibis?")
    ...
    val sol3 = conn.getConstraints.solveChocoDyn() // solveChocoX is now deprecated.
//    println("#######\n"+sol3)
    conn.update(sol3)
