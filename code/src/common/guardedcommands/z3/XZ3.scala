...
import common.guardedcommands.GuardedCom
import common.guardedcommands.FunAssgn
import common.guardedcommands.VarAssgn
import scala.Some
import common.guardedcommands.Pred
import common.guardedcommands.And
import common.guardedcommands.Seq
import common.guardedcommands.Or
import common.guardedcommands.Neg
import common.guardedcommands.Var
import common.guardedcommands.DataAssgn
import common.guardedcommands.Impl
import common.guardedcommands.NFunAssgn
import common.guardedcommands.IntAssgn
...
//      println("searching for data at "+index+" in "+data.mkString("[",",","]")+" or in "+fvals.mkString("[",",","]"))
    ...
    def xpred(p: common.Predicate, v:String) = {
...
//        println("testing if "+p+"(#"+n+")")
    ...
    def xfun(f: common.Function, v: String) = {
      fvars((f,v)) = fvarseed
      val varVal = variable
...
//      println("new valVar: "+varVal+" -> "+fvarseed)
    ...
    def xfun2(f: common.Function, vs: List[String]) = {
      fvars((f,vs)) = fvarseed
      val varVal = variable
      valVars(varVal) = fvarseed
      //      println("new valVar: "+varVal+" -> "+fvarseed)
...
//        theory.isEq(theory.getVar(v1),theory.getVar(v2))
      ...
      case FunAssgn(v1, v2, f: common.Function) =>
        z3.mkAnd(
          z3.mkEq(theory.getVar(v1),theory.xfun(f,v2)(theory.getVar(v2)))
        )
      case NFunAssgn(v1, v2s, nf: common.Function) =>
        if (v2s.size == 2)
