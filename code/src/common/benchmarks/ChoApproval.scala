...
import common.choco.dataconnectors._
import common.choco.{Var, ChoConstraints}
import common.Utils
import Utils._
...
object ChoApproval extends App {
  ...
  def genClients(n:Int): Iterable[ChoWriter] = {
    ...
    for (i <- 1 to n) {
      res ::=
        new ChoWriter("w"+i,0,List(AllApproval.join(i,(i*3 % 16)+5,(i*4 % 16)+5,(i*5 % 16)+5,(i*6 % 16)+5)))
